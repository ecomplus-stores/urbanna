import {
  ...
  export default {
    ...
    computed: {
      ...
      listAllGridsFiltered () {
        if (this.listGridsFiltered) {
            return this.listGridsFiltered
        }
      },
      ...
      diffObjectVariations () {
        const filteredObject = {}
        if (this.hasVariations && this.listGrids && this.listGridsFiltered) {
            for (const keyFiltered in this.listGridsFiltered) {
                if (Object.hasOwnProperty.call(this.listGridsFiltered, keyFiltered)) {
                    const filteredArray = this.listGridsFiltered[keyFiltered];
                    for (const keyAll in this.listGrids) {
                        if (Object.hasOwnProperty.call(this.listGrids, keyAll)) {
                            const notFilteredArray = this.listGrids[keyAll];
                            if (keyAll === keyFiltered) {
                                const diffArray = notFilteredArray.filter(value => !filteredArray.includes(value))
                                filteredObject[keyAll] = diffArray
                            }
                        }
                    }
                }
    ...
    methods: {
      ...
      listVariations (grids) {
        if (this.hasVariations && Object.keys(grids).length) {
            this.listGrids = grids
        }
      },

      listFilteredVariations (grids) {
        if (this.hasVariations && Object.keys(grids).length) {
            this.listGridsFiltered = grids
        }
      }
    },
